 ...
 *
 */
template<typename SquaredDeviations, typename D,
         std::enable_if<!std::is_same<D,DN>::value,int>::type=0>
void levenberg_marquardt(SquaredDeviations& ll, double lstep_down = 1.5,
                         double lstep_up = 5.0, double lmax = 1e9,
                         double lmin = 1e-9,
                         size_t nmax=100)
{
	typedef SquareMatrix<D> mat_t;

	/* Diagonal dampening matrix: */
	double l = 0.1;
	mat_t DTD(mat_t::diagonal(1.0));

	/* Parameter vector: */
	double cost_last = ll();
	double cost_min = cost_last;
	ColumnVector<D> p(ll.parameters());
	ColumnVector<DN> r(ll.data_count());
	ColumnVector<D> dp_last;
	ColumnVector<D> pmin(p);

	for (size_t i=0; i<nmax; ++i){
		/* Compute the proposed parameter step: */
		ColumnSpecifiedMatrix<D> J(ll.jacobian());
		//mat_t JTJ(J.T() * J);
		ColumnVector<D> dp = -(J.T()*J + l*DTD).inv() * (J.T() * r);
		...
		/* Evaluation of the acceptance criterion: */
		bool accept = (cost1 <= cost_last);
		if (!accept){
			/* Use the criterion (22) of Transtrum & Sethna (2012) to
			 * decide if we go uphill.
			 * beta is the cos of the angle between p and p1 (i.e.
			 * the dot product): */
			double n2 = dp.norm2() * dp_last.norm2();
			double beta = (n2 > 0) ? (dp * dp_last) / sqrt(n2) : -1;

			/* Eq. (22): */
			if ((1-beta)*cost1 < cost_min){
				/* Accept. */
				accept = true;
			}
		}

		std::cout << "  -- step " << i << ", accepting: " << accept
		          << ", dp=(" << dp[0] << ", " << dp[1] << ")\n";
